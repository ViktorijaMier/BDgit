...
<form method='get'>
<?PHP
	$file = fopen('uni-rezultatai.txt', 'a');
	$total = 10;
	$current = 1;
	$sum = 0;
	$sum2 = 0;
	...
	while($current <= $total){
		...
		if(!$connection){
			...
			Mysqli_select_db($connection, 'uni(1000)');
		}
...
// 1 SELECT:
		...
		$fetch = mysqli_query($connection, "SELECT post.Post_upload_date, type.Type_name, post.Title, image.Image, field_value.Value ". 
					...
					"INNER JOIN type ON post.Type_id = type.Type_ID ".
...
// 3 SELECT:
	   ...
	   $num=Mysqli_num_rows($fetch3);
			if($num>0){
			  
				for($i=0;$i<$num;$i++){
					
					$row = mysqli_fetch_row($fetch3);		
